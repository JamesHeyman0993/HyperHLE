...
fn NSClassFromString(env: &mut Environment, string: id) -> Class {
    ...
    // Using get_class instead of get_known_class to prevent panics
    // and allow the game to handle missing classes gracefully.
...
/// Internal helper to handle the logic for setting Objective-C properties.
fn perform_set_property(
    ...
    // Read old value (to release it later)
    ...
    // Store new value in the object's memory
...
// These wrappers make the functions available to the export macro
...
fn objc_setProperty_atomic(env: &mut Environment, _self: id, _cmd: SEL, new_value: id, offset: u32) {
    perform_set_property(env, _self, _cmd, new_value, offset);
}

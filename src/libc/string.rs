...
pub fn libcxx_string_init(env: &mut Environment, this_ptr: MutVoidPtr) {
    if this_ptr.is_null() {
        return;
    }
    // On 32-bit ARM LLVM libc++, an empty std::string (SSO layout) is 
    // initialized by zeroing out its internal structure (3 words / 12 bytes).
    for i in 0..12 {
        env.mem.write(this_ptr.cast::<u8>() + i, 0u8);
    }
}

pub fn libcxx_string_destructor(_env: &mut Environment, _this_ptr: MutVoidPtr) {
    // No-op safe: Since we initialize strings as empty, no allocation cleanup is required.
}

pub fn libcxx_string_assign(env: &mut Environment, this_ptr: MutVoidPtr, _src_ptr: ConstVoidPtr) -> MutVoidPtr {
    if !this_ptr.is_null() {
        for i in 0..12 {
            env.mem.write(this_ptr.cast::<u8>() + i, 0u8);
        }
